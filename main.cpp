...
public:
	...
	static void setAppKey(ttstr key) {
		*sAppKey = key;
	}

	static ttstr getAppKey() {
		return *sAppKey;
	}
	...
	static HGLOBAL createTJSData(tTJSVariant data) {
		...
		tTJSVariant container = createDictionary();
		ncbPropAccessor containerObj(container);
		containerObj.SetValue(L"appKey", *sAppKey);
		containerObj.SetValue(L"data", data);

		HGLOBAL unicodehandle = NULL;
		IStringWriter writer(0);
		writer.hex = true;
		getVariantString(container, &writer);
	...
	// TJS式のデータをデコードする
	static tTJSVariant extractTJSData(const tjs_char *tjs) {
		tTJSVariant container;
		tTJSVariant result;

		TVPExecuteExpression(tjs, &container);

		if (container.Type() != tvtObject)
			return result;

		ncbPropAccessor containerObj(container);
		ttstr appKey = containerObj.GetValue(L"appKey", ncbTypedefs::Tag<ttstr>());
		if (appKey != *sAppKey)
			return result;

		result = containerObj.GetValue(L"data", ncbTypedefs::Tag<tTJSVariant>());
